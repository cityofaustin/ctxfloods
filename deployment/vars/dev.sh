...
case $TRAVIS_EVENT_TYPE in
  push)
    ...
    export BUCKET_NAME=ctx-floods-frontend-$TRAVIS_BRANCH
    ;;
  pull_request)
    # Set to origin branch name during pull requests
    export BUCKET_NAME=ctx-floods-frontend-$TRAVIS_PULL_REQUEST_BRANCH
    ;;
  api | cron)
    ...
    export BUCKET_NAME=ctx-floods-frontend-$TRAVIS_EVENT_TYPE-$TRAVIS_JOB_ID
    ;;
esac
